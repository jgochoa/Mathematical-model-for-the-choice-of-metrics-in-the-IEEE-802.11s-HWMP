 ...
 *  parameters.
 *  Ademas este programa contiene  modificaciones realizadas por Juan Ochoa para su proyecto de tesis
 *  1. Se ha configurado para que genere un reporte cada segundo (m_inicio) 
 *  2. Se ha modificado el reporte con la finalidad de imprimir solo lo necesario, es decir las metricas
 */
...
MeshTest::MeshTest () :
  ...
  m_root ("ff:ff:ff:ff:ff:ff")//modo reactivo no existe mp_root
{
}
void
...
MeshTest::InstallApplication ()
{
  ...
  //este ciclo permite imprimir un reporte cada segundo
  for (Seconds(m_inicio);(Seconds(m_inicio) < Seconds(m_totalTime));m_inicio++)
  {    
        Simulator::Schedule (Seconds (m_inicio), &MeshTest::Report, this, aux);
...
MeshTest::Run ()
{
  ...
  CreateNodes ();
  //Configure (int argc, char *argv[]);//linea aumentada
  InstallInternetStack ();
  InstallApplication ();
  

  //Simulator::Schedule (Seconds (m_inicio), &MeshTest::Report, this);
...
void
MeshTest::Report (int cont)
{
  unsigned n (0);
  for (NetDeviceContainer::Iterator i = meshDevices.Begin (); i != meshDevices.End (); ++i, ++n)
    {
      std::ostringstream os;
      os << " mp-report-" << n <<"-"<< cont << ".xls";//modificado para obtener un archivo tipo base de datos
