...
"peerMeshPointAddress;"<< "metrica" << std::endl << m_peerMeshPointAddress << ";" */<< m_macPlugin->GetLinkMetric (m_peerAddress) <<        std::endl;
  
  //codigo que permite obtener la SNR 
  //WifiTxVector txVector;
 //"<PruebaSNR>" 
  //codigo que permite obtener la SNR 
  //<< m_macPlugin->GetWifiPhy ()->CalculateSnr(txVector,0.000001)<< std::endl;

//"SNR\";"<<mac->GetWifiPhy ()-> CalculateSnr(txVector,0.000001)<<std::endl 
  


  //"peerMeshPointAddress;\"" << m_peerMeshPointAddress << "\"" << std::endl <<
  //"metrica;\"" << m_macPlugin->GetLinkMetric (m_peerAddress) << "\"" << std::endl <<
  //"bandera=\"" << bandera+1 <<"\"" << std::endl <<
