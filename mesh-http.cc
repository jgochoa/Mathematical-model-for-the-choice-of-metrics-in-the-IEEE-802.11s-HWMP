...
//Trafico HTTP
  ...
  httpServerApps.Add (httpServer.Install (nodes.Get (0)));

  httpClientHelper httpClient (interface.GetAddress (m_xSize*m_ySize-1), httpServerPort);
  httpClientApps.Add (httpClient.Install (nodes.Get (m_xSize*m_ySize-1)));


  httpServerApps.Start (Seconds(0.0));
  httpServerApps.Stop (Seconds(m_totalTime));

  httpClientApps.Start (Seconds(0.0));
  httpClientApps.Stop (Seconds(m_totalTime));
*/
