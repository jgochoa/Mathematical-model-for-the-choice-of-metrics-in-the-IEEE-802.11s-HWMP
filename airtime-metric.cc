...
//codigo que permite obtener la SNR 
//uint32_t metric = (uint32_t)((double)(mac->GetWifiPhy ()-> CalculateSnr(txVector,0.000001)));
  
//Codigo original del calculo de alm
...
//uint32_t metric = (uint32_t)((double)( /*Overhead + payload*/
  ...
  //Codigo de prueba de metrica alm + snr
  //uint32_t metric = (uint32_t)((double)(mac->GetPifs () + mac->GetSlot () + mac->GetEifsNoDifs () + mac->GetWifiPhy ()->CalculateTxDuration (m_testFrame->GetSize (), txVector, mac->GetWifiPhy ()->GetFrequency())).GetMicroSeconds ()/(10.24 * (1.0 - failAvg)));

//calculo de DATA para trabajo de modelos lineales

//variable 1
//uint32_t metric = (uint32_t)((double)(mac->GetPifs ().GetMicroSeconds())); //valor constante

//variable 2
//uint32_t metric = (uint32_t)((double)(mac->GetSlot ().GetMicroSeconds()));//valor constante

//variable 3
//uint32_t metric = (uint32_t)((double)(mac->GetEifsNoDifs ().GetMicroSeconds()));//valor constante

//variable 4
//uint32_t metric = (uint32_t)((double)(mac->GetWifiPhy ()->CalculateTxDuration (m_testFrame->GetSize (), txVector, mac->GetWifiPhy ()->GetFrequency()).GetMicroSeconds ())); //variable valida
   
//variable 5
//uint32_t metric = (uint32_t)((double)(mac->GetWifiPhy ()-> CalculateSnr(txVector,0.000001))); //variable valida

//variable 6
//uint32_t metric = failAvg;//valor constante

//variable 7
//uint32_t metric = (uint32_t)((double)(mac->GetSifs ().GetMicroSeconds()));//valor constante

//variable 8
//uint32_t metric = (uint32_t)((double)(mac->GetWifiPhy ()->CalculatePlcpPreambleAndHeaderDuration(txVector).GetMicroSeconds()));//valor constante

//variable 8
//uint32_t metric = (uint32_t)((double)(mac->GetWifiPhy ()->GetNTxPower()));
